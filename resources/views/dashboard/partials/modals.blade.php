    ...
    <div id="user-management-modal" class="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 hidden overflow-y-auto p-4">
        <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-2xl w-full max-w-4xl overflow-hidden transform transition-all flex flex-col h-[600px]">
            <div class="p-6 md:p-8 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between">
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        <h3 class="text-2xl font-bold text-gray-800 dark:text-white">User Management</h3>
                        <p class="text-sm text-gray-500 dark:text-gray-400">Manage developer accounts and permissions</p>
                    </div>
                </div>
                <button id="close-user-management-modal" class="w-10 h-10 flex items-center justify-center rounded-xl hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-400 transition-all">
            ...
            <div class="flex-1 overflow-y-auto p-6 md:p-8 custom-scrollbar">
                ...
                <div id="user-list-view">
                    <div class="flex justify-between items-center mb-6">
                        <h4 class="text-lg font-bold text-gray-800 dark:text-white">All Users</h4>
                        <button id="add-new-user-btn" class="px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-xl text-sm font-bold transition-all flex items-center gap-2">
                    ...
                    <div id="users-list-container" class="space-y-4">
                        <!-- Dynamic user list content -->
                    </div>
                </div>

                <!-- Create/Edit User Form -->
                <div id="user-form-view" class="hidden">
                    <div class="flex items-center gap-3 mb-8 cursor-pointer text-gray-500 hover:text-purple-600 transition-colors" onclick="toggleUserView('list')">
                        <i class="fas fa-arrow-left"></i>
                        <span class="font-bold text-sm">Back to List</span>
                    </div>

                    <h4 id="user-form-title" class="text-xl font-bold text-gray-800 dark:text-white mb-6">Create New User</h4>
                    
                    <form id="admin-user-form" class="space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="space-y-2">
                                <label for="admin-user-name" class="block text-sm font-bold text-gray-700 dark:text-gray-300">Full Name</label>
                                <input type="text" id="admin-user-name" class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-2xl focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 transition-all text-sm font-medium" placeholder="e.g. John Doe" required>
                            </div>
                            <div class="space-y-2">
                                <label for="admin-user-email" class="block text-sm font-bold text-gray-700 dark:text-gray-300">Email Address</label>
                                <input type="email" id="admin-user-email" class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-2xl focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 transition-all text-sm font-medium" placeholder="john@example.com" required>
                            </div>
                        </div>

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="space-y-2">
                                <label for="admin-user-role" class="block text-sm font-bold text-gray-700 dark:text-gray-300">System Role</label>
                                <select id="admin-user-role" class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-2xl focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 transition-all text-sm font-medium appearance-none">
                                    <option value="developer">Developer</option>
                                    <option value="super_admin">Super Admin</option>
                                </select>
                            </div>
                            <div id="admin-user-status-container" class="space-y-2 hidden">
                                <label for="admin-user-status" class="block text-sm font-bold text-gray-700 dark:text-gray-300">Account Status</label>
                                <select id="admin-user-status" class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-2xl focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 transition-all text-sm font-medium appearance-none">
                        ...
                        <div id="password-display-section" class="hidden">
                            <div class="bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-900/30 rounded-2xl p-6">
                                <div class="flex items-start space-x-4">
                                    <div class="w-10 h-10 bg-amber-100 dark:bg-amber-900/20 text-amber-600 dark:text-amber-400 rounded-full flex items-center justify-center shrink-0">
                                        <i class="fas fa-lock text-sm"></i>
                                    </div>
                                    <div class="w-full">
                                        <h5 class="text-sm font-bold text-amber-800 dark:text-amber-300 mb-1">Generated Password</h5>
                                        <p class="text-xs text-amber-600 dark:text-amber-400 mb-4">Copy this password now. It will not be shown again.</p>
                                        <div class="flex items-center gap-2">
                                            <input type="text" id="generated-password-field" readonly class="flex-1 px-4 py-2.5 bg-white dark:bg-gray-800 border border-amber-200 dark:border-amber-900/40 rounded-xl text-lg font-mono font-bold text-center text-gray-800 dark:text-white">
                                            <button type="button" onclick="copyGeneratedPassword()" class="h-11 w-11 flex items-center justify-center bg-amber-600 hover:bg-amber-700 text-white rounded-xl shadow-lg shadow-amber-500/20 transition-all">
                        ...
                        <div class="pt-6 border-t border-gray-100 dark:border-gray-700 flex justify-end gap-3">
                            <button type="button" onclick="toggleUserView('list')" class="px-6 py-3 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-800 dark:text-gray-300 rounded-2xl font-bold text-sm transition-all">
                                Cancel
                            </button>
                            <button type="submit" id="save-user-btn" class="px-8 py-3 bg-purple-600 hover:bg-purple-700 text-white rounded-2xl shadow-lg shadow-purple-500/20 transition-all font-bold text-sm transform active:scale-[0.98]">
                                Create User
                            </button>
                            <button type="submit" id="update-user-btn" class="hidden px-8 py-3 bg-purple-600 hover:bg-purple-700 text-white rounded-2xl shadow-lg shadow-purple-500/20 transition-all font-bold text-sm transform active:scale-[0.98]">
                                Update User
                            </button>
                        </div>
                    </form>
